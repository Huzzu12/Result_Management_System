...
class resultClass:
    def __init__(self,root):
        ...
        lbl_makrs=Label(self.root,text="Marks Obtained",font=("goudy old style",20,"bold"),bg="white").place(x=50,y=280)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                messagebox.showerror("Error","Please first search student recourd",parent=self.root)
            else:
                ...
                if row!=None:
                    ...
                    cur.execute("insert into result (roll,name,course,makrs_ob,full_marks,per) values(?,?,?,?,?,?)",(
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_course.get(),
                        self.var_marks.get(),
